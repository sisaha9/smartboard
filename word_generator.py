...
while True: # Run forever
    ...
    if s != "": # If input is not empty
        ...
        time.sleep(3) # Wait for Arduino to process
        predictions_from_arduino = arduino.readline().decode() # Confirm Arduino's received predictions are correct
        if predictions_from_arduino: # If predictions from Arduino are not blank
            print(predictions_from_arduino[:-2]) # Print after stripping the \r\n
